$globalSchedule = {
	...
	var forScheduleUserName = '';
	var forScheduleUserPhone = '';
	var forScheduleUserId = '';
	var forScheduleUserEmail = '';
...
function newSugestionSchedule(indx, datass, profissional) {
	  ...
	  $.ajax({
        ...
        url: "/../jezzy-portal/dashboard/getFilteredSchecule",
...
function showServicePhoto(element){
	var src = $(element).attr('src');
			
	$("#img01Service").attr('src', src);
	
			 $('#myModalPhotoService').modal('toggle');
                        $('#myModalPhotoService').modal('show');
}


var contadorWelcomeModal = 0;
var welcomeModalPhotos = ["../jezzy-portal/img/firstLogin/welcome-1.jpg", "../jezzy-portal/img/firstLogin/welcome-2.jpg", "../jezzy-portal/img/firstLogin/welcome-3.jpg", "../jezzy-portal/img/firstLogin/welcome-4.jpg"];
...
$(document).ready(function() {
		...
		$(".nextHelper").click(function(){
			...
			console.log(contadorHelpersModal);
        ...
        url: "/../jezzy-portal/dashboard/isNotFirst",
        success: function(result) {
			introJs().start();
		...
		//fecha modal welcome
		$("#closeFirstLoginWelcome").click(function(){	
		
			  $('#myModalWelcome').modal('toggle');
			  $('#myModalWelcome').modal('hide');
			  introJs().start();
		...
		//muda fotos
		$("#nextFirstLoginWelcome").click(function(){
		
			
		
			if(contadorWelcomeModal == 3){
				$('#myModalWelcome').modal('toggle');
				$('#myModalWelcome').modal('hide');
			 ...
			 // $('#myModalHelper').modal('show');
				...
				contadorWelcomeModal++;
			}
			
			if(contadorWelcomeModal > 0){
				$("#prevFirstLoginWelcome").fadeIn(0);
			}

			
			
			var img = welcomeModalPhotos[contadorWelcomeModal];
			$("#firstLoginWelcome").attr("src", img);			
		
		});
		
		//muda fotos - volta
		$("#prevFirstLoginWelcome").click(function(){
			...
			if(contadorWelcomeModal > 0){
				...
				if(contadorWelcomeModal == 0){
					$("#prevFirstLoginWelcome").fadeOut(0);
				}
			}
		...
		$("#secondUserSchedule").change(function(){
			...
			var date = $("#dateSchecule").val();
				 ...
				 $.ajax({
                        ...
                        url: "/../jezzy-portal/dashboard/getSchedulesBySecondUser",
                        success: function(result) {

							$("#recebeHour").html(result);
							//alert(result);
		...
		/**
		VERIFICA SE EXISTEM HORÁRIOS CONFLITANTES PARA AGENDAMENTO
		**/
		$("#initialTimeSchecule").change(function(){
		
		
			var hour = $(this).val();
			var secondUserID = $("#secondUserSchedule").val();
			var date = $("#dateSchecule").val();
			 	 ...
			 	 $.ajax({
                        ...
                        url: "/../jezzy-portal/dashboard/verifySchedules",
                        success: function(result) {

                        if(result == "null"){
						
						 $("#initialTimeSchecule").removeClass("inputError");
							 $("#initialTimeSchecule").addClass("inputSuccess");
							 $("#showTimeErrorSchedule").html("");
							 $("#btnNewSchedule").fadeIn(200);
						}else{
						
							var jsonReturn = JSON.parse(result);
							
							var horarioInicio = jsonReturn[0].schedules.time_begin.substr(0, 5);
							var horarioTermino = jsonReturn[0].schedules.time_end.substr(0, 5);
							
							console.log(jsonReturn);
							$("#initialTimeSchecule").removeClass("inputSuccess");
							$("#initialTimeSchecule").addClass("inputError");
							
							$("#showTimeErrorSchedule").html("Temos um agendamento que pode conflitar com este horário: <strong>" + jsonReturn[0].schedules.subclasse_name + " com " + jsonReturn[0].schedules.client_name + " às " + horarioInicio+ "</strong><small> (previsão de término para às "+horarioTermino+")</small>");
							$("#btnNewSchedule").fadeOut(200);
						}
		...
		$("#emailSchedule").keyup(function(){
			...
			var email = $(this).val();
				 ...
				 $.ajax({
                        ...
                        url: "/../jezzy-portal/dashboard/verifyClientUserSchedules",
                        success: function(result) {
							
                        if(result == "null"){
						
							console.log("sucesso, NULO");
							 $("#emailSchedule").removeClass("inputError");
							 $("#emailSchedule").addClass("inputSuccess");
							 $("#btnNewSchedule").fadeIn(200);
							 $("#showEmailErrorSchedule").html("");
							 
						}else{
						
							var jsonReturn = JSON.parse(result);

								forScheduleUserName = jsonReturn[0].users.name;
								forScheduleUserPhone = jsonReturn[0].users.phone;
								forScheduleUserId = jsonReturn[0].users.id;
								forScheduleUserEmail = jsonReturn[0].users.email;
								
							console.log(jsonReturn);
							$("#emailSchedule").removeClass("inputSuccess");
							$("#emailSchedule").addClass("inputError");
							
							$("#showEmailErrorSchedule").html("Já existe um usuário cadastrado com esse endereço de email! <br/><strong><span class='cursor:pointer;' onclick='usignExistentUserByEmail()'>Desejo agendar para esse usuário</span></strong>");
							$("#btnNewSchedule").fadeOut(200);
						}
    ...
    //wizard counter 
    var wizardCounter = $("#firstLoginWizardCounter").val();
	...
	$("#change-password-btn").click(function(){
		...
		if(senha == confirmSenha){
			...
			$.ajax({
                    ...
                    url: "/../jezzy-portal/dashboard/changeCompanyPassword",
                    success: function(result) {
	...
	$("#secondary-wizard-btn").click(function(){
		...
		if(password == confirmPassword){
			...
			$.ajax({
                    ...
                    url: "/../jezzy-portal/dashboard/changeSecondaryUserPassword",
                    success: function(result) {
    ...
    $("#btnNewSchedule").click(function() {
        ...
        var chooseDay = $("#dateSchecule").val();
        if (chooseDay == "") {
            ...
            if (jsonReturn.length > 0) {
                ...
                $.each(jsonReturn, function(index, value) {
					...
					var isVoucher = '';
                       ...
                       var serviceNameLenght = value.schedules_solicitation.service_name.length;
                       if(serviceNameLenght<17) {
                            value.schedules_solicitation.service_name = value.schedules_solicitation.service_name + "<br>";
                       }
                       
                       console.log(value.schedules_solicitation.time_begin)


                       var div = "<div class='notification-item col-md-4' id='notification-item-" + index + "'><h4><a href='#' onclick='showUserDetail(" + value.schedules_solicitation.user_id + ")'>" + value.schedules_solicitation.user_name + "</a><br/><small>"+isVoucher+"</small></h4><hr /><span class='notification-text'>" + value.schedules_solicitation.service_name + "<br/><strong>" + findDayOfWeek(convertDate(value.schedules_solicitation.date)) + "</strong> - " + convertDate1(value.schedules_solicitation.date) + "" + "<br>às " + convertTime(value.schedules_solicitation.time_begin) + " </span><br/> <small>Profissional: "+ value.secondary_users.name +" </small><br/> <span class='glyphicon glyphicon-remove glyphicon-btn remove-solicitation-schedule' onclick='removeScheduleSolicitationDB(" + value.schedules_solicitation.id + "," + index + ");' ></span> <span class='glyphicon-btn'>|</span> <span class='glyphicon glyphicon-ok glyphicon-btn' onclick='aproveScheduleSolicitationDB(" + value.schedules_solicitation.id + "," + index + ")'></span><div class='label-new-schedule'>" +
	...
	*/
	$.ajax({
        type: "POST",
        data: {},
        url: "/../jezzy-portal/dashboard/getServicesToRealized",
        success: function(resultado) {

            var jsonReturno = JSON.parse(resultado);
			var htmlContentRealizeService = '';
           //console.log(jsonReturno);
			
				 if (jsonReturno.length > 0) {
					  $.each(jsonReturno, function(index, value) {
					  
                             console.log(value.schedules.subclasse_name);
                             if(value.schedules.subclasse_name.length<1) {
                                value.schedules.subclasse_name = "</br>";
                             } else if(value.schedules.subclasse_name.length < 19) {
                                value.schedules.subclasse_name = value.schedules.subclasse_name + "<br>";
                             }
						var myDiv = "<div class='col-md-4 realizeService text-center' id='setServiceRealized"+value.schedules.id+"'>"+
								"<p>"+
									...
									""+value.schedules.client_name+"<br/>"+
									""+convertTime(value.schedules.time_begin)+" <small>às</small> "+convertTime(value.schedules.time_end)+" <br/>"+ convertDate1(value.schedules.date) + "<br/>" +
    ...
    $("#createMoip").click(function() {
        ...
        if (x == true) {
            $.ajax({
                ...
                url: "/../jezzy-portal/dashboard/createMoIPAccount",
                success: function(result) {
...
function addNewUser() {
    ...
    return $.ajax({
        method: "POST",
        url: "https://secure.jezzy.com.br/jezzy-portal/user/ajaxAddNewUser",
        data: {User: {
                name: name,
                email: email,
                password: password,
...
function addNewSchedule(chooseDay) {
    ...
    return $.ajax({
        method: "POST",
        url: "https://secure.jezzy.com.br/jezzy-portal/schedule/ajaxAddSchedule",
        data: {Schedule: {
                schedulehour: time,
                serviceId: service,
                schedulePrice: price,
...
function findDayOfWeek(data) {
    //moment.locale('pt-BR');
    var dias_semana = new Array("Domingo", "Segunda",
            "Terça", "Quarta", "Quinta",
            "Sexta", "Sábado");
    ...
    //moment(stringDate).subtract(1,'d')
	var atualDate = moment(new Date).format('YYYY-MM-DD');

    //console.log(stringDate);
    //console.log(moment(stringDate).calendar());
    //console.log(moment(stringDate).weekday());
    ...
    if(moment(stringDate).date() == moment(new Date).date()) {
        ...
        return "Hoje";
    } else if(moment(stringDate).date() == moment(new Date).date()+1) {
        var dia = moment(new Date).day();
        return "Amanhã";
    } else {
        ...
        //console.log(moment(stringDate));
        //console.log(moment(stringDate).weekday(day).format('ddd'));
		//if(dia == 0){
		//	dia = 6;
		//}else{
		//	dia = dia - 1;
...
function removeScheduleSolicitation(indice) {
    ...
    $.ajax({
        ...
        url: "https://secure.jezzy.com.br/jezzy-portal/dashboard/writeFile",
        success: function(result) {
...
function aproveScheduleSolicitationDB(solicitationId, indice) {
    ...
    $.ajax({
        ...
        url: "https://secure.jezzy.com.br/jezzy-portal/dashboard/approveScheduleSolicitation",
        success: function(result) {
...
function removeScheduleSolicitationDB(solicitationId, indice) {
    ...
    $.ajax({
        ...
        url: "https://secure.jezzy.com.br/jezzy-portal/dashboard/reproveScheduleSolicitation",
        success: function(result) {
...
function suggestNewSchedule(indice, sid) {
    ...
    $.ajax({
        ...
        url: "https://secure.jezzy.com.br/jezzy-portal/dashboard/suggestNewSchedule",
        success: function(result) {
...
function convertDate(data){
    ...
    //dt = datass[0]+"/"+datass[1];
	return dt;
}

function convertDate1(data1){
    
    datass = data1.split("-");
    
    dt = datass[2]+"/"+datass[1];
    return dt;
}

function convertTime(inputTime){
    console.log(inputTime);
    timess = inputTime.split(":");
    tm = timess[0]+":"+timess[1];
    return tm;
}
...
window.onload = function() {
    ...
    if (firstLogin == 1) {
		...
		var isFirstToHelp = $("#dashboardFirstLogin").val();
 ...
 function setServiceNotRealized(id){
		...
		$("#setServiceRealized"+id).fadeOut();
		 $.ajax({
                ...
                url: "/../jezzy-portal/dashboard/setServiceNotRealized",
                success: function(result) {
...
function setServiceRealized(id){
		...
		$("#setServiceRealized"+id).fadeOut();
		 $.ajax({
                ...
                url: "/../jezzy-portal/dashboard/setServiceRealized",
                success: function(result) {
...
//	$("#myModalVoucherBody") -> irá receber população de dados
	 ...
	 $.ajax({
                ...
                url: "/../jezzy-portal/dashboard/showVoucherDetails",
                success: function(result) {
    ...
    $("#modelContentSuccess").addClass("textCenterModal");
    $("#modelContentSuccess").html('<span class="glyphicon glyphicon-ok" aria-hidden="true"></span> Agendamento salvo com sucesso');
    $('#divMessageScheduleSuccess').modal('show');
    return true;
}


function usignExistentUserByEmail(){
		
			 $("#userId").val(forScheduleUserId);
			$("#clientSchedule").val(forScheduleUserName);
			$("#phoneSchedule").val(forScheduleUserPhone);
			 $("#emailSchedule").val(forScheduleUserEmail);
			     $("#newUserSchedule").prop("checked", false);
