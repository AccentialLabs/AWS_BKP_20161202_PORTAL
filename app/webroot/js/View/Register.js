function selectUserFromIndication(element){

			var id = $(element).attr("id");
			var name = $(element).attr("name");
			var responsibleEmail = $(".registerCompanyResponsibleEmail").val();
			var companyEmail = $(".registerCompanyEmail").val();
			
			$("#indicationUserID").val(id);
			
			$("#meuMenuDropDown").toggle();
			$("#meuMenuDropDown").fadeOut();			
			$("#dropdownMenuButton").val(name);
				...
				$.ajax({			
					...
					url: "/../jezzy-portal/company/searchIndicationCodeByUser",
					success: function(result){	

						console.log(result);
					$("#indicationCode").val(result.trim());
					$("#indicationUserID").val($("#indicationUserID").val()+"-"+result);
					$("#errorFromIndicationCode").fadeOut();
					$("#indicationCode").attr('disabled','disabled');
		...
		//busca usuários por nome/ Indicacao
		$("#dropdownMenuButton").keyup(function(){
		
			var name = $("#dropdownMenuButton").val();
			...
			$.ajax({			
					...
					url: "/../jezzy-portal/company/searchUsersByNameToDropDown",
					success: function(result){	
		...
		$("#meuMenuDropDown").mouseleave(function(){
		
			$("#meuMenuDropDown").slideUp();
		
		});
		
		$(".isPhoto").change(function(){
				...
				var email = $(".registerCompanyEmail").val();
					...
					success: function(result){	
						...
						if(result == false){
							...
							/**
							* Caso o EMAIL seja válido, então verificamos se já existe
							*  algum secondary user registrado com o mesmo email
							*/
									$.ajax({			
					type: "POST",			
					data:{
					...
					url: "/../jezzy-portal/company/verificaEmailCompanyInSecondUserTable",
					success: function(result){	
