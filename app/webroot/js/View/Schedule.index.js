		...
		*/
            $.ajax({
                ...
                if (msg !== 0 && msg !== "0") {
					...
					/**
		RECUPERA TEMPO DO SERVICE
		*/
			 $.ajax({
                method: "POST",
                url: getControllerPath("schedule") + "ajaxGetServiceDuration",
                data: {Schedule: {
                        serviceId: $("#serviceSchedule").val()
                    }
                }
            }).done(function (msg) {
                var inicio = $("#initialTimeSchecule").val();
				inicio = inicio.split(":");
				var fim = new Date();
				fim.setHours(inicio[0]);
				fim.setMinutes(inicio[1]+msg);
				

				
				var myDate = new Date(fim).toTimeString().replace(/.*(\d{2}:\d{2}:\d{2}).*/, "$1");
 $("#endTimeSchecule").val(myDate);
 $("#endDateFormGroup").fadeIn(300);
            });
