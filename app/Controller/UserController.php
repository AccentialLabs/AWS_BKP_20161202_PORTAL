	...
	class UserController extends AppController {
		...
		public function index() {
			...
			$this->set('secundaryUsersFees', $this->getSecondaryUsersCommissioningFees());
		...
		public function isNotFirst(){
			$this->autoRender = false;
			 $this->Session->write('userFirstLogin', 0);
		
		}
		
		public function getEditSecondUserCommissionament(){
		$this->layout = "";
			$id = $this->request->data['id'];
			
			$sql = "SELECT * FROM secondary_users_commissioning_fees WHERE id = {$id};";
				...
				$register = $this->AccentialApi->urlRequestToGetData('users', 'query', $params);
				$this->set('commissioningFee', $register);
		}
		
		public function updateSecondUserCommissionament(){
			$this->autoRender  = false;
			
			$perCompany = $this->request->data['rate_per_company_product'];
			$perJezzy = $this->request->data['rate_per_jezzy_product'];
			$perService =  $this->request->data['rate_per_service'];
			$id = $this->request->data['commissionament_id'];
			
			$sql = "UPDATE secondary_users_commissioning_fees 
			SET rate_per_company_product = {$perCompany}, 
			rate_per_jezzy_product = {$perJezzy}, 
			rate_per_service = {$perService} 
			WHERE id = {$id};";
		...
		public function addSecondaryUver() {
			...
			if ($this->request->is('post')) {
				...
				$query = "INSERT INTO secondary_users(name, email, password, company_id, secondary_type_id, first_login, color)"
						. " VALUES('" . $param['SecondaryUser']['name'] . "'"
						. ",'" . $param['SecondaryUser']['email'] . "'"
						. ",'" . $param['SecondaryUser']['hashPass'] . "'"
						. "," . $param['SecondaryUser']['company_id'] . ""
						. "," . $param['SecondaryUser']['type'] . ", 0, '".$color."');
...
VALUES(
@LAST_ID,
{$this->request->data['SecondaryUser']['ratePerCompanyProduct']},
{$this->request->data['SecondaryUser']['ratePerJezzyProduct']},
{$this->request->data['SecondaryUser']['rate_per_service']});
...
'REGISTRATION_PUSHBOTS_ID');
		...
		*/
		...
		private function getSecondaryUsersCommissioningFees(){
			
			$company = $this->Session->read('CompanyLoggedIn');
			
			$sql = "SELECT * FROM secondary_users_commissioning_fees
inner join secondary_users on secondary_users.id = secondary_users_commissioning_fees.secondary_user_id
WHERE secondary_users.company_id = {$company['Company']['id']} and secondary_users.excluded = 0;";
			$params = array(
				'User' => array(
					'query' => $sql
				)
		...
		private function getSecundaryUsers($companyId) {
			$secondUserSQL = "select * "
					...
					. "inner join secondary_users_commissioning_fees on secondary_users_commissioning_fees.secondary_user_id = secondary_users.id "
		...
		public function editSecondaryUser() {
			...
			// ---------------------
				$query = "UPDATE secondary_users_commissioning_fees 
					SET
					rate_per_company_product = ". $this->request->data['SecondaryUser']['perCompany'] .",
					rate_per_jezzy_product = ". $this->request->data['SecondaryUser']['perJezzy'] .",
					rate_per_service = ". $this->request->data['SecondaryUser']['perService'] ." 
					 WHERE secondary_user_id = " . $this->request->data['SecondaryUser']['id'] . ";";
	...
	<tr style="background: #f7f7f7;">
			<td colspan="4">
				<br/>
				<img src="https://secure.jezzy.com.br/uploads/Emails/files/transacao-finalizada/04.jpg" />
				<br/>
			</td>
		</tr>
		<tr style="background: #f7f7f7; width: 800px;">
			...
			<td style="width: 150px; text-align: right;" colspan="1">
				<a href="#"><img src="https://secure.jezzy.com.br/uploads/Emails/files/transacao-finalizada/App Store.png" width="80"/></a>
			</td>
		...
		<tr>
			<td colspan="4">
				<img src="https://secure.jezzy.com.br/uploads/Emails/files/transacao-finalizada/05-1.jpg" height="30" width="800" style="vertical-align: bottom;"/>
			</td>
		</tr>
		<tr>
			<td colspan="4">
				<img src="https://secure.jezzy.com.br/uploads/Emails/files/transacao-finalizada/06.jpg"  style="vertical-align: bottom;"/>
			</td>
		</tr>
		<tr>
	...
	// Limpa os destinatários e os anexos
		...
		public function createAssociatedUserSolicitation(){
				...
				$sqlSelect = "SELECT * FROM associated_users
				inner join associated_user_solicitations on associated_user_solicitations.requester_associated_user_id = associated_users.id
				WHERE associated_users.id = {$associatedUserId} and associated_user_solicitations.requester_company_id = {$company['Company']['id']};";
				$paramSelect = array(
				'User' => array(
					'query' => $sqlSelect
					)
				...
				$associated = $this->AccentialApi->urlRequestToGetData('users', 'query', $paramSelect);
				
				
				$url = "https://secure.jezzy.com.br/jezzy-portal/utils/solicitationAcceptAssociatedUser?solicitationID=".$associated[0]['associated_user_solicitations']['id'];
				
				$this->sendSolicitationaEmailAssociatedUser($associated[0]['associated_users']['name'], $associated[0]['associated_users']['email'], $url, $company['Company']['fancy_name']);
		...
		"SELECT (SELECT  count(*) FROM schedules_solicitation inner join secondary_users on secondary_users.id = schedules_solicitation.secundary_user_id WHERE  schedules_solicitation.company_id = 100028 and MONTH(schedules_solicitation.date_solicitation) = 8  and secondary_users.id = 111) AS total_mensal,(SELECT  count(*) FROM schedules_solicitation inner join secondary_users on secondary_users.id = schedules_solicitation.secundary_user_id WHERE  schedules_solicitation.company_id = 100028 and secondary_users.id = 111 and MONTH(schedules_solicitation.date_solicitation) = 8 AND DAYOFMONTH(schedules_solicitation.date_solicitation) = 22) AS total_passado,(SELECT  count(*) FROM schedules_solicitation inner join secondary_users on secondary_users.id = schedules_solicitation.secundary_user_id WHERE  schedules_solicitation.company_id = 100028 and secondary_users.id = 111 and MONTH(schedules_solicitation.date_solicitation) = 8 AND YEARWEEK(schedules_solicitation.date_solicitation)=YEARWEEK(NOW())) AS total_semanal;";
			...
			$varrrr = $this->AccentialApi->urlRequestToGetData('users', 'query', $param);
			print_r($varrrr);
		...
		$this->autoRender = false;
			...
			$enviado = $this->sendSolicitationaEmailAssociatedUser("Matheus Odilon", "matheusodilon0@gmail.com", "ESSE_E_O_LINK_CLICK_AQUI", "Comp 12212");
		
		}
		
		public function sendSolicitationaEmailAssociatedUser($name, $email, $url, $comp) {
			$mail = new PHPMailer(true);
	...
	// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
			...
			<meta name="viewport" content="width=device-width, initial-scale=1.0">
				...
				<tr style="background: #f2f2f2;">
					<td colspan="4">
						<br/>
				<center>
					<span style="color: #999933; font-family: Helvetica, Arial, sans-serif; font-size: 36px;"><i>Olá, '.$name.'!</i></span>
				</center>
				<br/><br/>
			</td>
		</tr>
		<tr style="background: #f2f2f2;">
			<td colspan="4">
		<center>
			<span style="color: #9b9b9b; font-family: Helvetica, Arial, sans-serif; font-size: 24px;">O salão '.$comp.' está lhe convidado para fazer parte da sua equipe.</span>
		</center>
		<br/>
	</td>
	</tr>
	<tr style="background: #f2f2f2; text-align: center;">
		<td colspan="4" style=" font-family: Helvetica, Arial, sans-serif; font-size: 16px;">
			<br/>
			<span style="color: #2597ac;">Para aceitar clique no link abaixo:</span><br/>
			<span style="color: #9b9b9b;"><a href="'.$url.'"> ' . $url . '</a></span><br/>
			<br/>
		</td>
	</tr>
	<tr style="background: #f7f7f7;">
	...
	// Limpa os destinatários e os anexos
		...
		public function solicitationAcceptAssociatedUser(){
			...
			*/
			...
			UPDATE
			*/
			$sqlUpdate = "UPDATE associated_user_solicitations SET `status` = 'ACCEPTED'  WHERE id = {$solicitatioId} ;";
			$paramUpdate = array(
						'User' => array(
								'query' => $sqlUpdate
						)
...
function random_color() {
	$color1 =  $this->random_color_part();
	$color2 =  $this->random_color_part();
	$color3 =  $this->random_color_part();
