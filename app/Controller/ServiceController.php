...
class ServiceController extends AppController {
	...
	public function isNotFirst(){
			$this->autoRender = false;
			 $this->Session->write('serviceFirstLogin', 0);
            ...
            foreach ($post as $postKey => $postValue) {
                if (substr($postKey, -1) != "0") {
                    ...
                    if (!empty($postValue['value']) && !empty($postValue['time']) && count($postValue['user']) > 0) {
                        $result = $this->editService($postValue, $company);
                    } else {
                        $result = $this->removeService($postValue, $company);
                    }
					
                } else {
                    if (!empty($postValue['value']) && !empty($postValue['time']) && count($postValue['user']) > 0) {
                        $result = $this->addService($postValue, $company);
                    }
                }
            }
            if ($result) {
