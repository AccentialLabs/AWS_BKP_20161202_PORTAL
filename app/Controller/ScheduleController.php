...
class ScheduleController extends AppController {
    ...
    public function ajaxAddSchedule() {
        ...
        if ($this->request->is('post')) {
			...
			/**
		ENVIA NOTIFICACAO DE AGENDAMENTO CONFIRMADO COM SUCESSO
		*/
		//$msg = "Agendamento confirmado! ".$service['subclasses']['name'];
		
		//$Dashboard = new DashboardController();
		//$Dashboard->sendPublicMobileNotification($this->request->data['Schedule']['userId'], "Agendamento confirmado!");
	...
	public function ajaxUpdateSchedule(){
		$this->autoRender = false;
		  ...
		  if ($this->request->is('post')) {
            ...
            $queryDelete = "UPDATE schedules SET 
			`date` = '".$this->request->data['scheduleDate']."', 
			`time_begin` = '".$this->request->data['scheduleTimeBegin']."',
			`time_end` =  '".$this->request->data['scheduleTimeEnd']."' WHERE id = " . $this->request->data['scheduleID'];
	...
	public function ajaxSimulateAPIRequest(){
		
		$this->autoRender = false;
		
		  $queryDelete = "SELECT * FROM schedules inner join secondary_users on secondary_users.id = schedules.secondary_user_id;";
            ...
            $schedules = $this->AccentialApi->urlRequestToGetData('schedules', 'query', $scheduleParam);
			
			echo base64_encode(json_encode($schedules));
     ...
     * @return int
     */
    public function ajaxGetServiceDuration() {
        $this->autoRender = false;
        if ($this->request->is('post')) {
            if ($this->request->data['Schedule']['serviceId'] == 0) {
                return 0;
                ...
                $company = $this->Session->read('CompanyLoggedIn');
                $query = "SELECT time FROM services WHERE id = '" . $this->request->data['Schedule']['serviceId'] . "' AND companie_id = '" . $company['Company']['id'] . "'";
                $params = array(
                    'Service' => array(
                        'query' => $query
                    )
				...
				$retorno = $this->AccentialApi->urlRequestToGetData('Services', 'query', $params);
                return $retorno[0]['services']['time'];
            }
        }
        return 0;
    }
	...
	public function getUserByName(){
		...
		$query = "select * from companies_users
inner join users on users.id = companies_users.user_id
where companies_users.company_id = " . $company['Company']['id'] . " and companies_users.status = 'ACTIVE' and users.name LIKE '{$name}%';";
